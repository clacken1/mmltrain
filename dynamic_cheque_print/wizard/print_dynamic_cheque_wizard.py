...
class PrintChequeWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if payment:
            ...
            raise UserError(
                _('Payment Not fount You cannot print Cheque !!!!'))

    cheque_format = fields.Many2one(
        'dynamic.cheque', string='Cheque Format', required=True)
    ...
    def print_dynamic_cheque_report(self):
        ...
        return self.env.ref('dynamic_cheque_print.dynamic_cheque_print_report_action').report_action(self)

    @api.model
    def _create_paper_format(self):
        report_action_id = self.env['ir.actions.report'].search(
            [('report_name', '=', 'dynamic_cheque_print.report_dynamic_check_print')])
        if not report_action_id:
            raise Warning(
                'Someone has deleted the reference view of report, Please Update the module!')
        ...
        self._cr.execute(
            """ DELETE FROM report_paperformat WHERE name='Custom Report'""")
